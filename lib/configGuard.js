...
/** Own version, for the backup filename. Read lazily so a test stub can skip it. */
function pkgVersion() {
    try {
...
/** Filename-safe form of a semver string — mirrors the frontend's rule. */
...
function createConfigGuard(adapter, opts = {}) {
    ...
    // Version that wrote the backup — same filename/payload field the frontend
    // uses, so the settings list labels guard backups like its own.
    const version = String(opts.version || adapter.version || pkgVersion() || '');
    ...
    async function writeBackup(short, prevRaw, from) {
        ...
        const payload = { _ts: ts, _version: version, _changed: [store], _details: details };
        ...
        const stem = `backup-${stamp}${version ? `-v${versionForFilename(version)}` : ''}`;
        const name = `${stem}.json.gz`;
        await adapter.writeFileAsync(`${ns}.backups`, name, gz);
        await adapter.writeFileAsync(
            `${ns}.backups`,
            `${stem}.meta.json`,
            JSON.stringify({ _ts: ts, _version: version, _changed: payload._changed, _details: details }),
        );
        await prune();
        return name;
    }
