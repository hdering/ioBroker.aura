...
function measureWidget(widget, input) {
    ...
    if (counted && Number.isFinite(out.items)) {
        ...
        out.requiredPx = Math.round(minimumPx(minimum, widget && widget.type, pres));
...
function renderMeasure(list, input) {
    ...
    // The two settings the measured heights had to be re-computed for. Said up
    // front, because otherwise the numbers below look like they came straight
    // out of the metrics file — and on this dashboard they did not.
    if (!isReferencePresentation(pres)) {
        head.push(
            `Darstellung dieses Dashboards: Schriftskalierung ${pres.fontScale}, Innenabstand ` +
                `${pres.widgetPadding} px. Die gemessenen Höhen (bei ${pres.refFontScale} und ` +
                `${pres.refPaddingPx} px gemessen) sind darauf umgerechnet.`,
        );
    }
