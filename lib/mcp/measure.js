...
function measureWidget(widget, input) {
    ...
    const counted =
        (metrics && metrics.counted && metrics.counted[type]) ||
        (alias && Number.isFinite(out.items) ? metrics && metrics.counted && metrics.counted[alias] : null);
    ...
    if (counted && Number.isFinite(out.items)) {
        ...
        out.unknown = notMeasurable;
    } else {
        out.unknown = 'für diesen Typ ist keine Messung hinterlegt';
    }
