...
function rowShape(counted, widget) {
    ...
    return { basePx: Math.round(basePx), perItemPx: Math.round(perItemPx * 10) / 10, applied, rowTypes };
}
...
function rowTypeSurcharge(rowTypes, widget, items) {
    ...
    const options = (widget && widget.options) || {};
    const entries = Array.isArray(options.entries) ? options.entries.slice(0, items) : null;
    ...
    const displays = entries
        ? entries.filter((e) => !(e && e.divider === true)).map((e) => rowDisplayType(e, options))
...
function itemCount(widget) {
    ...
    // A cap turns a runtime row count into a known one — that is the whole point
    // of maxRows on autolist and statusoverview, whose rows are otherwise only
    // countable once the dashboard is running.
    const cap = Number.isFinite(o.maxRows) && o.maxRows > 0 ? Math.floor(o.maxRows) : null;
...
function capped(widget) {
    const o = (widget && widget.options) || {};
    return Number.isFinite(o.maxRows) && o.maxRows > 0;
}
...
function measureWidget(widget, input) {
    ...
    if (counted && Number.isFinite(out.items)) {
        ...
        // measured default, a contact or a state chip is taller. Per row, because a
        // list mixes them.
        const rowsExtra = rowTypeSurcharge(shape.rowTypes, widget, out.items);
        out.requiredPx = Math.round(shape.basePx + out.items * shape.perItemPx + rowsExtra.px);
        out.basis =
            `${shape.basePx} px + ${out.items} × ${shape.perItemPx} px/${counted.item}` +
            `${rowsExtra.px ? ` + ${rowsExtra.px} px Darstellung (${rowsExtra.notes.join('; ')})` : ''}` +
