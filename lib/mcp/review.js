...
/** Units and id fragments that mark a rising meter rather than a measurement. */
const COUNTER_UNITS = new Set(['kwh', 'wh', 'm³', 'm3', 'l', 'kvarh', 'mwh']);
 ...
 */
function reviewWidgets(widgets) {
    ...
    if (tiles.length >= TILE_ROW_LIMIT) {
        const devices = new Set(tiles.map((w) => parentOf(w.datapoint)).filter(Boolean));
        add(
            'tile-row',
            `${tiles.length} Einzelkacheln für je einen Datenpunkt`,
            [
                'Das ist der Fall, für den es die Listen-Widgets gibt: eine autolist übernimmt sie aus Raum und',
                `Gewerk und nimmt neue Geräte von selbst auf, eine list bildet dieselben ${tiles.length} Zeilen`,
                'mit je eigenem Bedienelement ab.',
                ...(devices.size > 1 ? [`Betroffen sind ${devices.size} Geräte.`] : []),
            ].join(' '),
            tiles.length > 8 ? 'raum-liste' : 'geraete-liste',
            tiles,
        );
    }
    ...
    const counters = list.filter((w) => {
        if (w.type !== 'value') {
            return false;
        }
        const unit = String(opts(w).unit || '').toLowerCase();
        return COUNTER_UNITS.has(unit) || COUNTER_WORDS.test(String(w.datapoint || ''));
    });
    if (counters.length) {
        add(
            'counter-as-reading',
...
module.exports = { renderReview, reviewWidgets, CONTACT_LIMIT, TILE_ROW_LIMIT };
