...
function effectiveSettings(frontend, layout, section) {
    ...
    return {
        ...
        gridWidthMode: three('gridWidthMode'),
        fluidDesignWidth: three('fluidDesignWidth'),
...
/**
 * The fluid grid (#413, setting gridWidthMode 'fluid'): the frontend keeps the column
 * count and stretches the columns to the window (utils/gridColumns.ts). A column is
 * then no longer snapX px wide — gridPos.x/w are SHARES of the design width, heights
 * stay in px. With fluidDesignWidth 0 the design is whatever the section's widgets
 * occupy, so there is no right edge to run past: `designCols` is null then.
 */
function widthModeOf(settings) {
    const s = settings || {};
    const fluid = s.gridWidthMode === 'fluid';
    const grid = gridOf(s);
    const designWidth = fluid && Number(s.fluidDesignWidth) > 0 ? Number(s.fluidDesignWidth) : 0;
    return {
        widthMode: fluid ? 'fluid' : 'fixed',
        designCols: designWidth > 0 ? pxToCols(designWidth, grid) : null,
    };
}

/** The fluid grid as the sentence that goes into a tool answer ('' on the fixed grid). */
function renderWidthMode(canvas) {
    if (!canvas || canvas.widthMode !== 'fluid') {
        return '';
    }
    return (
        'Breite: Modus "Fensterbreite füllen" — die Spalten werden auf die Fensterbreite gestreckt (Höhen ' +
        'bleiben in px). gridPos.x/w sind Anteile der Entwurfsbreite, ' +
        (canvas.designCols
            ? `die ${canvas.designCols} Spalten umfasst (fluidDesignWidth); was schmaler bleibt, lässt rechts anteilig Platz.`
            : 'und die ist die belegte Breite des Bereichs (rechteste Widget-Kante über alle Tabs) — der Inhalt ' +
              'füllt also immer die volle Breite, eine rechte Grenze gibt es nicht. Nebeneinander stehende ' +
              'Widgets sollten daher gemeinsam bis zur selben rechten Spalte reichen.') +
        ' Die Pixelbreiten unten gelten für die Entwurfsbreite; im Browser sind sie gestreckt oder gestaucht.'
    );
}
...
function designCanvas(input) {
    ...
    const { widthMode, designCols } = widthModeOf(s);
    ...
    return {
        ...
        // The fluid grid has no device edge on the right — only its design width,
        // and with the automatic one (0) not even that.
        maxCols: widthMode === 'fluid' ? designCols : pxToCols(usableWidth, grid),
...
function renderCanvas(canvas) {
    const fluidNote = renderWidthMode(canvas);
    ...
    return (
        ...
        (canvas.maxCols
            ...
            : `. Auf den Bildschirm passen ${canvas.maxRows} Zeilen — `) +
