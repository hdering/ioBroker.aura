    ...
    for (const place of places) {
        ...
        for (const widget of widgets) {
            ...
            if (datapointMeta) {
                ...
                historyFindings(widget, datapointMeta).forEach((m) => noHistory.push(`${at}: ${m}`));
...
function renderAudit(result, scope) {
    ...
    const head =
        `${stats.widgets} Widget(s) in ${stats.places} Tab(s)/Ansicht(en), ${stats.datapoints} ` +
        'Datenpunkt-Verweis(e) geprüft.';
    if (!findings.length) {
