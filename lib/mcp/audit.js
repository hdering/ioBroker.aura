...
const { canvasColorFindings, entryDisplayFindings, validateWidget } = require('./validate');
...
function auditDashboard(input) {
    ...
    for (const place of places) {
        ...
        for (const widget of widgets) {
            ...
            // A token in a colour eCharts paints on a canvas: it resolves to
            // nothing there, so the series is invisible. Stored dashboards carry
            // it because the palette used to say "always var(--token)".
            canvasColorFindings(widget, { themeValues }).forEach((m) => canvasColors.push(`${at}: ${m}`));
    ...
    add(
        'canvas-colors',
        `${canvasColors.length} Serienfarbe(n), die eCharts nicht lesen kann`,
        'Ein var(--token) wird auf dem Canvas nicht aufgelöst (auch nicht mit Fallback) — die Serie wird in ' +
            'der letzten gesetzten Farbe gezeichnet und ist auf dunklem Grund unsichtbar. Der Befund nennt ' +
            'den Wert, der stattdessen hineingehört.',
        canvasColors,
    );
    add(
        'display-mismatch',
