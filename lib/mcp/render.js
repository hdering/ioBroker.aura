...
function typeName(o, depth = 0) {
    if (o.enum) {
        return o.enum.map((v) => `"${v}"`).join(' | ');
    }
    if (o.tuple) {
        return `[${o.tuple.map((t) => typeName(t, depth + 1)).join(', ')}]`;
    }
    ...
    if (o.type === 'array') {
        return `${o.items ? typeName(o.items, depth) : 'any'}[]`;
    }
    // An inline object type — written out, not called "object". Reported from
    // use: `contactAppearance` answered `object` here, so the labels of a
    // contact row ("Offen"/"Zu") could not be looked up and the way to rename a
    // state was to reach for the `states` mapping instead. Two levels deep is
    // enough for every literal in the schema and keeps the line readable.
    if (o.fields && depth < 2) {
        const parts = Object.entries(o.fields).map(
            ([k, f]) => `${k}${f.required ? '' : '?'}: ${typeName(f, depth + 1)}`,
        );
