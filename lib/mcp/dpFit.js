...
function historyFindings(widget, meta, available) {
    ...
    for (const read of historyReads(widget)) {
        ...
        if (!info.logging.length) {
            warnings.push(
                `${read.path}: für "${read.id}" ist kein History-Adapter aktiviert (kein history./influxdb./sql.-` +
                    'Eintrag in common.custom) — die Auswertung bleibt dauerhaft leer. Im ioBroker am Objekt die ' +
                    'Aufzeichnung einschalten, oder einen Datenpunkt nehmen, der geloggt wird.' +
                    (installed && installed.length ? ` Vorhandene Instanzen: ${installed.join(', ')}.` : ''),
            );
            continue;
        }
        ...
        if (ghosts.length === info.logging.length && ghosts.length) {
            warnings.push(
                `${read.path}: "${read.id}" ist auf ${ghosts.join(', ')} eingetragen, aber diese Instanz gibt es ` +
                    'in dieser Installation nicht — es wird nichts aufgezeichnet, und eine History-Abfrage darauf ' +
                    'läuft ins Leere.' +
                    (installed.length ? ` Vorhanden ist ${installed.join(', ')}.` : ' Es ist gar keine installiert.'),
            );
            continue;
        }
