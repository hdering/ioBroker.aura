 ...
 *
 * `states` used to be in here and does NOT belong: StateDisplay draws a chip out
 * of the value mapping and writes nowhere. On a room of smoke and water sensors
 * — read-only states, mapped for display, exactly what the display is for — that
 * was nine findings, every one of them wrong. `contact`, `time` and `value` are
 * read displays for the same reason and were never in the list.
 */
...
function writeRefs(widget) {
    ...
    // A fill widget's limits are dragged by the user, so each one writes to its own
    // datapoint (#613). A limit on a read-only state draws a handle that moves and
    // then silently snaps back — precisely the failure this module exists to catch.
    if (widget && widget.type === 'fill' && o.limitsEditable !== false) {
        const limits = Array.isArray(o.limits) ? o.limits : [];
        limits.forEach((limit, i) => {
            if (!limit || typeof limit !== 'object' || limit.editable === false) {
                return;
            }
            const name = typeof limit.label === 'string' && limit.label ? ` „${limit.label}“` : '';
            push(limit.datapoint, `Grenze ${i + 1}${name}`, 'Grenze');
        });
    }
    ...
    if (widget && (widget.type === 'list' || widget.type === 'autolist')) {
        ...
        entries.forEach((entry, i) => {
            ...
            // `writable: false` is the row saying it does not write — the whole
            // point of the flag. Warning that its control sits on a read-only
            // datapoint told the author what they had already declared.
