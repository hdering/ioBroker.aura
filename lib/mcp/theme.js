...
/**
 * token → the value(s) it has on THIS dashboard, for the places a token cannot
 * be used at all.
 *
 * A canvas colour (echartSeries[].color) has to be the value itself, and a
 * finding that only says "not a token here" leaves the caller to fetch it. With
 * "follow the browser" two themes are in play and the token has two values —
 * both are named, because a canvas colour cannot follow the theme either way.
 */
...
function renderPalette(tokens, choice) {
    ...
    return [
        ...
        ...lines,
        // The one exception, said here rather than left to be discovered: the
        // instruction above is what puts a token into a canvas colour, and there
        // it resolves to nothing. echartSeries[].color was the reported case.
        'Ausnahme eCharts: echartSeries[].color zeichnet auf ein Canvas und löst keine CSS-Variable auf — ' +
            'dort den Wert selbst eintragen (die Liste oben nennt ihn).',
...
function renderTheme(tokens, choice, opts = {}) {
    ...
    out.push(
        'Eine Ausnahme: `echartSeries[].color` zeichnet auf ein Canvas. Dort löst der Browser keine ' +
            'CSS-Variable auf (auch nicht mit Fallback), die Serie bleibt unsichtbar — hier gehört der Wert ' +
            'aus der Liste unten direkt in die Konfiguration.',
    );
