...
const { measureWidget, renderMeasure } = require('./measure');
...
const INSTRUCTIONS = [
    ...
    '   is left wondering why the setting did nothing. Read its warnings, not only the errors:',
    ...
    '7. aura_measure whenever you set a height for a list, table, gauge or chart. Rows are not',
    '   pixels, and a list cut off after nine of sixteen rows looks correct in the JSON.',
...
const TOOLS = [
    ...
    },
    {
        ...
        description:
            ...
            'to in weeks, options the widget does not read any more, settings one level too high, duplicate ' +
            'widget ids, groups whose children are gone, empty tabs. Without a tab it sweeps every tab, popup ' +
            'and group in the dashboard, which is where the health half earns its keep. Suggestions, not edits.',
        inputSchema: {
            type: 'object',
            properties: {
                tab: {
    ...
    },
    {
        ...
        description:
            ...
            'height for a list, a table, a gauge or a chart.',
        inputSchema: {
            type: 'object',
            properties: {
                ...
                items: {
                    ...
                    description:
                        ...
                        'overview): with 16 the answer is for 16 rows.',
                },
                layout: { type: 'string' },
                section: { type: 'string' },
            },
    ...
    },
    {
        ...
        description:
            ...
            'history adapter records, a switch on a read-only state, a slider without a range. Run this before ' +
            'every write.',
        inputSchema: {
            type: 'object',
            properties: {
 ...
 */
async function locateWidget(adapter, a) {
    const layouts = await readDashboard(adapter);
    const defs = await readGroupDefs(adapter);
    const id = a.widgetId;
 ...
 */
async function resolveTargetHost(adapter, layouts, a, key) {
    const needle = a[key];
    ...
    if (!found.error) {
        return {
            ...
            label: `${found.tab.layoutName} / ${found.tab.sectionName} / ${found.tab.name}`,
        };
    }
    if (!view.error) {
        return {
    ...
    const popupNames = views.map((v) => `- Popup ${v.name}`);
...
async function runTool(name, args, ctx) {
    ...
    switch (name) {
        ...
        case 'aura_review': {
            ...
            // One tab, or everything. Without a tab the style half would be a wall
            // of the same four remarks per tab, so the whole-dashboard sweep is the
            // health check — the one that gets more useful the bigger the dashboard.
            let places;
            let scope;
            ...
            if (mode === 'style' || mode === 'both') {
                if (!styleWidgets) {
                    ...
                    for (const place of places) {
                        const findings = reviewWidgets(place.widgets);
                        if (findings.length) {
                    ...
                    parts.push(renderReview(reviewWidgets(styleWidgets), scope));
                }
            }
            ...
            if (mode === 'health' || mode === 'both') {
                ...
                const defs = await readGroupDefs(adapter);
                const groupPlaces = Object.entries(defs).map(([defId, children]) => ({
                    where: `Gruppe ${defId}`,
                    widgets: Array.isArray(children) ? children : children && children.widgets,
                }));
                ...
                const [knownDatapoints, stateValues, datapointMeta] = await Promise.all([
                    listStateIds(adapter),
                    readStateValues(adapter, refs),
                    // The objects too, for the history check and the fit check. Same
                    // ids, so the sweep costs one more read per datapoint and can
                    // then answer "why is this chart empty".
                ...
                const orphanDefIds = layouts.length ? Object.keys(defs).filter((id) => !referencedDefs.has(id)) : [];
                ...
                parts.push(
                    renderAudit(
                        auditDashboard({
                            ...
                            datapointMeta,
                            defs,
                            orphanDefIds: a.tab ? undefined : orphanDefIds,
        ...
        case 'aura_measure': {
            const grid = await readGrid(adapter);
            let widgets;
            let where;
            let url;
            if (a.json) {
            ...
            const rows = widgets.map((w) => measureWidget(w, { metrics: ctx.metrics, grid, items: a.items }));
            return text(renderMeasure(rows, { grid, where, url, metrics: ctx.metrics }));
        }
        ...
        case 'aura_validate': {
            ...
            if (a.checkDatapoints !== false) {
                ...
                const widgets = Array.isArray(parsed.value?.tab?.widgets)
                    ? parsed.value.tab.widgets
                    : Array.isArray(parsed.value?.widgets)
                      ? parsed.value.widgets
                      : [parsed.value];
                const refs = new Set();
                for (const widget of widgets) {
                    for (const ref of collectDatapointRefs(widget, schema)) {
                        refs.add(ref.id);
                    }
                }
                if (refs.size) {
                    vctx.datapointMeta = await readStateMeta(adapter, refs);
                    note += ` ${vctx.datapointMeta.size} Objekt(e) gelesen.`;
                }
            }
            const layouts = await readDashboard(adapter);
            if (layouts.length) {
        ...
        case 'aura_write_tab': {
            ...
            const { errors, warnings } = validateTab(
                { _type: 'aura-tab', tab: { name: found.label, widgets } },
                schema,
                {
                    knownDatapoints,
                    // A popup has its own grid, so the dashboard width does not apply.
                    ...(found.kind === 'tab' ? { columns: designColumns(layouts) } : {}),
                    strictIndices,
                },
            );
            if (errors.length) {
        ...
        case 'aura_copy_widget': {
            ...
            const check = await validateWidgets(adapter, schema, nextTarget, target.label, {
                ...(target.kind === 'tab' ? { columns: designColumns(layouts) } : {}),
        ...
        case 'aura_insert_preset': {
            ...
            const check = await validateWidgets(adapter, schema, nextWidgets, target.label, {
                ...(target.kind === 'tab' ? { columns: designColumns(layouts) } : {}),
        ...
        case 'aura_update_widget': {
            ...
            const knownDatapoints = await listStateIds(adapter);
            const { errors, warnings } = validateTab(
                { _type: 'aura-tab', tab: { name: host.label, widgets: nextList } },
                schema,
                {
                    knownDatapoints,
                    strictIndices: [index],
                    // Tabs share the dashboard grid; groups and popups have their own.
                    ...(host.kind === 'tab' ? { columns: designColumns(host.layouts) } : {}),
                },
            );
            if (errors.length) {
