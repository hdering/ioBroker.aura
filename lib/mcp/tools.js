...
const { measureWidget, renderMeasure, renderRendered } = require('./measure');
const { designCanvas, renderCanvas } = require('./canvas');
const { readThemeChoice, renderPalette, renderTheme, themeValues } = require('./theme');
...
const INSTRUCTIONS = [
    ...
    'Reality check: aura_rendered gives the heights the BROWSER measured on a tab that was open — what it',
    'rendered, what the content needs, what scrolls. aura_measure works off a table measured once, which',
    'ages with every change to the styling; where the two disagree, the browser is right. A tab nobody',
    'has opened has no measurement — ask the user to open it.',
    ...
    'off silently, and aura_measure can then size it.',
...
const TOOLS = [
    ...
    },
    {
        ...
        description:
            ...
            'change to the styling. Where both exist the answer names the difference. Only tabs somebody ' +
            'has had OPEN since the adapter started have a measurement (a hidden tab renders nothing), and ' +
            'every entry says how old it is — ask the user to open the tab if the one you need is missing.',
        inputSchema: {
            type: 'object',
            properties: {
...
async function validateWidgets(mcp, widgets, label, extra) {
    ...
    ctx.themeValues = themeValues(mcp.themeTokens, await readThemeChoice(adapter));
...
async function runTool(name, args, ctx) {
    ...
    switch (name) {
                ...
                for (const section of layout.sections || []) {
                    ...
                    rows.push(
                        `- ${layout.name} / ${section.name}` +
                            ...
                            (differs ? ` [Platz hier: ${cv.maxCols} Spalten × ${cv.maxRows} Zeilen]` : ''),
                    );
                    for (const tab of section.tabs || []) {
                        ...
                        const budget = cv.enabled
                            ? ends > cv.maxRows
                                ...
                                : `, endet auf Zeile ${ends} von ${cv.maxRows}`
                            : ends
                ...
                parts.push(
                    renderAudit(
                        auditDashboard({
                            ...
                            datapointMeta,
                            loggingInstances,
                            themeValues: themeValues(ctx.themeTokens, await readThemeChoice(adapter)),
                            defs,
                            orphanDefIds,
        ...
        case 'aura_measure': {
            ...
            const est = new Map(rows.map((m) => [m.id, m.requiredPx]));
            const off = (live.widgets || []).filter((w) => {
                const e = est.get(w.id);
                return w.scrolls || (Number.isFinite(e) && e > 0 && Math.abs(w.contentPx - e) > 8);
            });
            return text(
                [
                    answer,
                    ...
                    `Der Browser hat diesen Tab wirklich gezeichnet (${new Date(live.ts).toISOString().slice(11, 16)} ` +
                        ...
                        (off.length
                            ? ` Bei ${off.length} Widget(s) stimmt die Schätzung oben nicht mit dem Gezeichneten ` +
                              'überein oder es scrollt — aura_rendered zeigt beides nebeneinander.'
                            : ' Nichts scrollt, und die Schätzungen oben decken sich mit dem Gezeichneten.'),
                ].join('\n'),
            );
        }
        ...
        case 'aura_rendered': {
            ...
            if (a.tab) {
                ...
                asked = found.label;
                wanted = wanted.filter((id) => id === found.tab.id);
            }
            if (!wanted.length) {
                return text(
                    [
                        ...
                        'Gemessen wird der Tab, der gerade offen ist: den Nutzer bitten, ihn im Frontend zu ' +
                            'öffnen (nicht im Editor — der Editor meldet nichts, seine Vorschau ist schmaler ' +
                            'als das Dashboard). Nach etwa einer Sekunde steht die Messung bereit.',
            ...
            for (const tabId of wanted) {
                ...
                for (const w of report.widgets || []) {
                    ...
                    estimates[w.id] = m.requiredPx;
                }
                list.push({ report, estimates });
            }
            list.sort((x, y) => (y.report.ts || 0) - (x.report.ts || 0));
            return text(renderRendered(list, { metrics: ctx.metrics }));
        }
        ...
        case 'aura_validate': {
            ...
            // caller to aura_theme.
            const vctx = { themeValues: themeValues(ctx.themeTokens, await readThemeChoice(adapter)) };
