...
const INSTRUCTIONS = [
    ...
    '7. aura_write_tab or aura_add_widget. Every write backs the configuration up first.',
    '',
    ...
    'Review: aura_review looks over a tab that already exists and names what would make it better — the',
    'answer to "why does my dashboard look so bare". Offer it when the user asks about an existing tab.',
...
const TOOLS = [
    ...
    },
    {
        ...
        level: 'read',
        description:
            'Looks over a tab that already exists and names what would make it a better dashboard: rows of ' +
            'single-value tiles that belong in one list, numbers without a good or bad range, a meter shown as ' +
            'its raw reading, a list with no second line, a tab where nothing reacts to anything. Each finding ' +
            'names the widgets and the recipe that fixes it. Suggestions, not edits — show them to the user and ' +
            'change only what they want.',
        inputSchema: {
            type: 'object',
            properties: {
                tab: { type: 'string', description: 'Tab or popup view: name, slug or id.' },
                layout: { type: 'string' },
                section: { type: 'string' },
            },
            required: ['tab'],
...
async function runTool(name, args, ctx) {
    ...
    switch (name) {
        ...
        case 'aura_review': {
            ...
            if (found.error) {
                const names = allTabs(layouts).map((t) => `- ${t.layoutName}/${t.sectionName}/${t.name}`);
                return fail([found.error, 'Vorhanden:', ...names].join('\n'));
            }
            const where = `${found.tab.layoutName} / ${found.tab.sectionName} / ${found.tab.name}`;
            return text(renderReview(reviewWidgets(found.tab.widgets), where));
        }
