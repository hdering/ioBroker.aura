...
async function locateWidget(adapter, a) {
    ...
    const inTab = findWidget(layouts, id);
    if (!inTab.error) {
        return {
            kind: 'tab',
            layouts,
            defs,
            tab: inTab.tab,
            list: inTab.tab.widgets,
            index: inTab.index,
            label: `${inTab.tab.layoutName} / ${inTab.tab.sectionName} / ${inTab.tab.name}`,
        };
    }
    ...
    // Last: a group child whose defId the caller did not name. Cheap to find and
    // it saves a round trip through aura_group for the id it already has.
    for (const [defId, children] of Object.entries(defs)) {
        const index = (children || []).findIndex((w) => w && w.id === id);
        if (index >= 0) {
            return { kind: 'group', layouts, defs, defId, list: children, index, label: `Gruppe ${defId}` };
        }
    }
...
async function callTool(name, args, ctx) {
    ...
    switch (name) {
        ...
        case 'aura_insert_preset': {
            ...
            const layouts = await readDashboard(adapter);
            const target = await resolveTargetHost(adapter, layouts, a, 'tab');
            if (target.error) {
                return fail(target.error);
            }
            ...
            return text(
                [
                    `Vorlage „${found.preset.name}“ als "${widget.id}" in ` +
                        `${target.tab.layoutName} / ${target.tab.sectionName} / ${target.label} eingefügt.`,
                    `Sicherung: ${adapter.namespace}.backups/${backup}`,
                    EDITOR_NOTE,
                    ...(check.warnings.length ? ['', formatFindings([], check.warnings)] : []),
                ].join('\n'),
