...
const INSTRUCTIONS = [
    ...
    '',
    'Writing can get things wrong. Show the user what you intend to change before',
    ...
    'Any widget can show extra values in its header: options.headerItems, each a datapoint, a value the',
    'widget already has (main value, list sum/count) or a text with bindings, on a slot beside the title or',
    'in a second header row, expanded and/or folded (defaultCollapsed) — recipe "eingeklappt-mit-wert".',
...
const TOOLS = [
    ...
    },
    {
        ...
        description:
            ...
            'conditions, a status overview, a whole room tab, a countdown that switches a device on for a ' +
            'while (runs in the adapter, scriptable through its cmd state). Without an id it lists what is there. Read ' +
...
async function resolveDefId(adapter, a, defs) {
    ...
    // A released protected view has to look like an ordinary tab here too, or the
    // group widget that holds the defId is invisible and the answer below claims
    ...
    if (!widget) {
        // Same reason as in locateWidget: a stub without a release has no widgets
        // to search, so the lock gets named instead of the id doubted.
        const locked = lockedHostOf(adapter, allTabs(layouts), a.widgetId);
        if (locked) {
            return { error: releaseHint(locked.label), pinLocked: true };
        }
...
async function locateWidget(adapter, a, model) {
    ...
    if (!hits.length) {
        // Not a phantom id: it may well exist and only sit in a protected view,
        // where the model carries an empty stub. „Gibt es nicht“ was the answer
        // for an id aura_tab had printed itself — the lock is the reason, and it
        // is the same refusal aura_compact gives (resolveTargetHost).
        const locked = lockedHostOf(adapter, allTabs(layouts), id);
        if (locked) {
            return { error: releaseHint(locked.label), pinLocked: true };
        }
...
async function tabGridCtx(adapter, layouts, where) {
    ...
    return {
        ...
        ...(canvas.enabled
            ? { maxCols: canvas.maxCols, maxRows: canvas.maxRows, canvas }
            : canvas.widthMode === 'fluid'
              ? { canvas }
              : {}),
    };
}
                    ...
                    rows.push(
                        `- ${layout.name} / ${section.name}` +
                            ...
                            (differs && !secLock.locked
                                ? ` [Platz hier: ${cv.maxCols ? `${cv.maxCols} Spalten × ` : ''}${cv.maxRows} Zeilen]`
                                : '') +
                            (cv.widthMode === 'fluid' ? ' [Breite: füllt das Fenster]' : ''),
                    );
                    for (const tab of section.tabs || []) {
