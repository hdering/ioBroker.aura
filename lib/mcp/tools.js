...
const INSTRUCTIONS = [
    ...
    '   follows the layout and the options you set (a card row is twice a default one, a second line',
    '   per entry adds half again) — measure the widget you are about to write, not a plain one.',
...
const TOOLS = [
    ...
    },
    {
        ...
        description:
            ...
            'height for a list, a table, a gauge or a chart. A list row is measured per layout and per option ' +
            '(second line, header), and the answer names the factors that are NOT in the number. With ' +
            'guidelines set it also names the widgets that end outside the target screen.',
        inputSchema: {
            type: 'object',
            properties: {
...
async function runTool(name, args, ctx) {
    ...
    switch (name) {
        ...
        case 'aura_dashboard': {
            const [layouts, frontend, logging] = await Promise.all([
                readDashboard(adapter),
        ...
        case 'aura_measure': {
            ...
            if (a.json) {
                ...
                if (found.kind === 'tab') {
                    ...
                    // screen guidelines say nothing about it.
                    canvas = { enabled: false, grid: canvas.grid };
                }
            } else {
            ...
            return text(renderMeasure(rows, { grid, where, url, metrics: ctx.metrics, canvas }));
        }
        ...
        case 'aura_write_tab': {
            ...
            const { errors, warnings } = await validateWidgets(adapter, schema, widgets, found.label, {
        ...
        case 'aura_update_widget': {
            ...
            const { errors, warnings } = await validateWidgets(adapter, schema, nextList, host.label, {
                strictIndices: [index],
