...
function entryDisplayFindings(widget) {
    ...
    for (const rule of ENTRY_DISPLAY_FIELDS) {
        ...
        for (const { where, entry } of rows) {
            ...
            if (rule.reads.includes(display)) {
                continue;
            }
            const set = rule.fields.filter((f) => !isUnset(entry[f]));
            if (!set.length) {
                continue;
            }
...
module.exports = { allowedOptions, entryDisplayFindings, validateAny, validateTab, validateWidget };
