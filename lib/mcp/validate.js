...
/**
 * A colour the browser resolves in CSS but not on a canvas.
 *
 * `ctx.fillStyle = 'var(--accent)'` is DROPPED — measured in the real browser,
 * fallback included (`var(--accent, #3b82f6)` too): the value stays whatever was
 * set last, so the series is painted in the previous colour and on a dark chart
 * disappears. `currentColor` behaves the same way.
 */
const CANVAS_DEAD_COLOR = /var\(\s*--|^\s*currentColor\s*$/i;

/**
 * Colour options that end up on a canvas, where a CSS variable is not a colour.
 *
 * Only eCharts renders that way (`renderer: 'canvas'` in EChartWidget); every
 * other widget paints DOM, where a token is exactly right — and the palette in
 * `aura_dashboard` says so in one unconditional sentence, which is what puts a
 * token here in the first place. Reported from use: a series colour of
 * `var(--accent)` and a chart that stayed empty for good.
 */
const CANVAS_COLOR_FIELDS = [{ type: 'echart', option: 'echartSeries', field: 'color', what: 'Serienfarbe' }];

/**
 * Findings for a token in a colour the canvas cannot read.
 *
 * `ctx.themeValues` (token → value on this dashboard) turns the finding into a
 * fix: without it the caller is told to ask aura_theme instead.
 */
function canvasColorFindings(widget, ctx) {
    const out = [];
    const options = (widget && widget.options) || {};
    for (const rule of CANVAS_COLOR_FIELDS) {
        ...
        options[rule.option].forEach((item, i) => {
            ...
            if (typeof value !== 'string' || !CANVAS_DEAD_COLOR.test(value)) {
                return;
            }
            const token = (value.match(/--[\w-]+/) || [])[0];
            const known = token && ctx.themeValues ? ctx.themeValues.get(token) : null;
            out.push(
                `Option "${rule.option}[${i}].${rule.field}": "${value}" ist auf dem Canvas keine Farbe — ` +
                    `eCharts zeichnet die ${rule.what} dort, wo CSS-Variablen nicht aufgelöst werden (auch ` +
                    'nicht mit Fallback), und die Serie bleibt unsichtbar. ' +
                    (known
                        ? `Den Wert eintragen: ${token} ist auf diesem Dashboard ${known}.`
                        : 'Den Wert selbst eintragen (#rrggbb) — aura_theme nennt ihn zum Token.'),
            );
        });
    }
    return out;
...
function validateWidget(widget, schema, ctx = {}) {
    ...
    // A token in a canvas colour. An ERROR like every other unusable value: the
    // chart renders and shows nothing, which is the class of mistake this
    // validator exists for, and the fix is the value the finding names.
    canvasColorFindings(widget, ctx).forEach(err);
