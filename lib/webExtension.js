...
function rewriteHtml(html, baseUrl) {
    ...
    const wsSnippet  = `<script>(function(){` +
        // rw(url): rewrite same-origin non-/aura/ requests and explicit target-origin
        // requests to go through /aura/proxy.
        `var tgt=${JSON.stringify(targetOrigin)},pp=${JSON.stringify(PROXY_PATH)};` +
        `function rw(u){` +
            `try{` +
            ...
            `return u;` +
        `}` +
        // WebSocket: app constructs path as location.pathname + 'ws/...' which
        // merges into /aura/proxyws/... — detect and rewrite to /aura/proxyws?url=
        `var _W=window.WebSocket;` +
        `window.WebSocket=function(u,p){` +
            `try{` +
                `var a=new URL(u.replace(/^wss?:/,'https:'),location.href);` +
                `if(a.origin===location.origin&&a.pathname.indexOf(pp)===0){` +
                    `var path=a.pathname.slice(pp.length);` +
