...
export function detectType(dp: DatapointEntry): { type: WidgetType; unit?: string } {
    ...
    if (
        role.includes('thermostat') ||
        role === 'level.temperature' ||
        role.includes('temp.set') ||
        name.includes('solltemp') ||
        name.includes('setpoint') ||
        name.includes('set_temperature')
    )
    ...
    if (
        role.includes('level.blind') ||
        role.includes('level.roller') ||
        role === 'blind' ||
        role === 'roller' ||
        (unit === '%' &&
            (name.includes('rollade') ||
                name.includes('rollo') ||
    ...
    if (
        role.includes('level.dimmer') ||
        role.includes('level.brightness') ||
        name.includes('dimmer') ||
        name.includes('helligkeit')
    )
        return { type: 'dimmer' };

    // Power / energy
    if (unit === 'W' || unit === 'VA' || role.includes('value.power')) return { type: 'value', unit };
    if (unit === 'kWh' || unit === 'Wh' || role.includes('value.energy')) return { type: 'value', unit };
    ...
    if (role.includes('sensor.window') || role.includes('sensor.door')) return { type: 'windowcontact' };

    // Read-only binary indicators: lowbat, maintenance, motion, alarm, lock, …
    if (
        role.startsWith('indicator') ||
        role.includes('sensor.motion') ||
        role === 'motion' ||
        role.includes('sensor.alarm') ||
        role.includes('sensor.lock') ||
        role.includes('lowbat')
    )
        return { type: 'binarysensor' };

    // Any other read-only boolean → binary sensor
    if (dp.write === false && dp.type === 'boolean') return { type: 'binarysensor' };

    // Writable boolean / explicit switch/button role
    if (dp.type === 'boolean' || role.includes('switch') || role.includes('button')) return { type: 'switch' };

    // Generic numeric value
    if (dp.type === 'number') return { type: 'value', unit: unit || undefined };
...
const KEYWORDS_NORM: Array<[string, string[]]> = [
    ...
    [
        'sicherheit',
        ['security', 'alarm', 'door', 'window', 'motion', 'contact', 'tuer', 'fenster', 'bewegung', 'rauch', 'smoke'],
    ],
...
function scoreNorm(
    ...
    for (const [key, s] of KEYWORDS_NORM) {
        if (key === nt || nt.startsWith(key) || key.startsWith(nt)) {
            syns = s;
            break;
        }
    }
...
export function scoreDatapoint(dp: DatapointEntry, topic: string): number {
    return scoreNorm(norm(topic), dp.funcs, dp.rooms, norm(dp.name), norm(dp.id), norm(dp.role ?? ''));
}
...
export function detectWidgets(datapoints: DatapointEntry[], topic: string, maxItems = 500): DetectedWidget[] {
    const nt = norm(topic);
