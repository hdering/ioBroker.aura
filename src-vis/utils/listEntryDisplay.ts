...
export function getRoleDisplay(role: string | undefined, val: unknown): RoleDisplay | null {
    ...
    if (r === 'motion' || r.startsWith('sensor.motion') || r.includes('presence')) {
        return on ? { label: 'Bewegung', color: '#f59e0b' } : { label: 'Ruhig', color: 'var(--accent-green)' };
    }
    ...
    if (r.includes('vibration')) {
        return on ? { label: 'Vibration', color: '#f59e0b' } : { label: 'Ruhig', color: 'var(--accent-green)' };
    }

    // Doorbell
    if (r.includes('doorbell') || r.includes('bell')) {
        return on ? { label: 'Klingelt', color: '#f59e0b' } : { label: 'Ruhig', color: 'var(--text-secondary)' };
    }
