...
function FlowArrow({ active, color, reverse = false, power = 0, scale = 1 }: {
  active: boolean; color: string; reverse?: boolean; power?: number; scale?: number;
}) {
  ...
  return (
    <div className="flex-1 relative flex items-center" style={{ height: h, overflow: 'hidden' }}>
      <div className="w-full" style={{ height: line, background: active ? color : 'var(--app-border)', opacity: 0.45 }} />
      {active && (
        <div style={{
          position: 'absolute',
...
function VertFlowArrow({ active, color, power = 0, down = true, scale = 1 }: {
  active: boolean; color: string; power?: number; down?: boolean; scale?: number;
}) {
  const dur = active ? Math.max(0.5, 2.5 - (power / 5000)) : 2;
  const H = 24 * scale;
  const W = 10 * scale;
  ...
  return (
    <svg width={W} height={H} style={{ overflow: 'visible' }}>
      <line x1={cx} y1={0} x2={cx} y2={H}
        ...
        strokeWidth={Math.max(1, 1.5 * scale)} strokeOpacity={0.45} />
      {active && (
    ...
    <div className="flex flex-col">
      ...
      <div className="flex items-center">
        <div className="flex flex-col items-center" style={{ width: colW, gap: 2 * scale }}>
          ...
          <span className="font-semibold tabular-nums" style={{ color: '#f59e0b', fontSize: valFs }}>{fmtKW(site.pvPower)}</span>
        ...
        <div className="flex flex-col items-center" style={{ width: colW, gap: 2 * scale }}>
          ...
          <span className="font-semibold tabular-nums" style={{ color: 'var(--text-primary)', fontSize: valFs }}>{fmtKW(site.homePower)}</span>
          <span style={{ color: 'var(--text-secondary)', fontSize: labelFs }}>{t('evcc.house')}</span>
        </div>

        <FlowArrow active={gridImport || gridExport} color={gridColor} reverse={gridImport} power={Math.abs(site.gridPower)} scale={scale} />

        <div className="flex flex-col items-center" style={{ width: colW, gap: 2 * scale }}>
          <Zap size={iconSz} color={gridColor} />
          <span className="font-semibold tabular-nums" style={{ color: gridColor, fontSize: valFs }}>{fmtKW(Math.abs(site.gridPower))}</span>
      ...
      {showBattery && (
        <div className="flex flex-col items-center">
          <VertFlowArrow active={battActive} color={battColor} power={Math.abs(site.batteryPower)} down={battCharge} scale={scale} />
          <div className="flex items-center rounded-xl" style={{
            ...
            <span className="font-bold tabular-nums" style={{ color: battColor, fontSize: battFs }}>{fmtSoc(site.batterySoc)}</span>
            {site.batteryPower !== 0 && (
              <span style={{ color: 'var(--text-secondary)', fontSize: battSub }}>· {fmtKW(Math.abs(site.batteryPower))}</span>
            )}
            {site.batteryMode && site.batteryMode !== 'normal' && site.batteryMode !== 'unknown' && (
              <span style={{ color: 'var(--text-secondary)', fontSize: battSub }}>· {site.batteryMode}</span>
            )}
          </div>
        </div>
      )}
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full" style={{ gap: 12 * scale }}>
      ...
      <div style={{ width: '100%', maxWidth: barW }}>
        <div className="relative rounded-full overflow-hidden" style={{ height: barH, background: 'var(--app-border)' }}>
...
function SocBar({ current, target, color, scale = 1 }: { current: number; target: number; color: string; scale?: number }) {
  return (
    <div className="relative rounded-full overflow-hidden" style={{ height: 8 * scale, background: 'var(--app-border)' }}>
      ...
      {target > 0 && target < 100 && (
        <div className="absolute top-0 h-full"
          style={{ left: `${target}%`, width: Math.max(1, 2 * scale), background: 'var(--text-primary)', opacity: 0.6 }} />
      )}
    </div>
  );
}
...
function LoadpointCard({ lp, idx, prefix, compact, scale = 1 }: {
  lp: LoadpointState; idx: number; prefix: string; compact: boolean; scale?: number;
}) {
  const t = useT();
      ...
      <div className="flex items-center flex-wrap" style={{ gap: 8 * scale, fontSize: fs }}>
        ...
        {lp.charging && (
          <span className="flex items-center gap-1" style={{ color: accent }}>
            <span className="inline-block rounded-full animate-pulse" style={{ background: accent, width: dot, height: dot }} />
            {fmtKW(lp.chargePower)}
          </span>
        )}
        <span className="ml-auto font-medium flex items-center gap-1" style={{ color: 'var(--text-secondary)' }}>
          {pendingMode !== null && (
            <span className="inline-block rounded-full animate-pulse" style={{ background: 'var(--text-secondary)', width: dot, height: dot }} />
          )}
  ...
  return (
    <div className="border-t" style={{
      ...
      <div className="flex items-center" style={{ gap: 8 * scale }}>
        ...
        <span className="font-semibold flex-1 truncate" style={{ color: 'var(--text-primary)', fontSize: titleFs }}>
          {isHeating ? lpTitle : (lp.connected ? vehicleName : lpTitle)}
        </span>
        {lp.charging && (
          <span className="flex items-center gap-1" style={{ color: accent, fontSize: valFs }}>
            <span className="inline-block rounded-full animate-pulse" style={{ background: accent, width: chargeDot, height: chargeDot }} />
            {t(isHeating ? 'evcc.heating' : 'evcc.charging')}
          </span>
        )}
        {lp.chargePower > 0 && (
          <span className="font-bold" style={{ color: accent, fontSize: valFs }}>{fmtKW(lp.chargePower)}</span>
        )}
      </div>

      {!isHeating && lp.connected && lp.vehicleSoc > 0 && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 4 * scale }}>
          <div className="flex items-center justify-between" style={{ color: 'var(--text-secondary)', fontSize: labelFs }}>
            ...
            <div className="flex items-center gap-1">
              ...
              <button className="font-semibold hover:opacity-80 flex items-center gap-0.5" style={{ color: '#6366f1' }}
                ...
                {pendingLimitSoc !== null && (
                  <span className="inline-block rounded-full animate-pulse" style={{ background: '#6366f1', width: pendingDot, height: pendingDot }} />
                )}
      ...
      {lp.charging && (
        <div className="flex items-center" style={{ color: 'var(--text-secondary)', gap: 12 * scale, fontSize: subFs }}>
        ...
        {MODES.map((m) => {
          ...
          return (
            <button key={m.key} onClick={() => setMode(m.key)}
              ...
              {pending && <span className="inline-block rounded-full animate-pulse bg-white" style={{ width: pendingDot, height: pendingDot }} />}
              {m.label}
            </button>
          );
        })}
...
export function EvccWidget({ config }: WidgetProps) {
  ...
  const autoScale = autoScaleEnabled
    ? Math.max(autoMin, Math.min(autoMax, boxSize.w / REF_W))
    : 1;
  ...
  const headerEl = (showTitle || showIcon) ? (
    <div className="flex items-center shrink-0 min-w-0" style={{ gap: 6 * headerScale }}>
      ...
      {showTitle && <p className="truncate flex-1 min-w-0"
        style={{ color: 'var(--text-secondary)', textAlign: titleAlign as React.CSSProperties['textAlign'], fontSize: headerFs }}>
        {config.title}
      </p>}
    </div>
  ) : null;
  ...
  if (layout === 'battery') return (
    <div ref={boxRef} className="aura-widget-row flex flex-col h-full">
      {headerEl && <div style={{ marginBottom: 4 * headerScale }}>{headerEl}</div>}
      <div className="flex-1 min-h-0"><BatteryView site={site} scale={mainScale} /></div>
    </div>
  );
  ...
  if (layout === 'production') return (
    <div ref={boxRef} className="aura-widget-row flex flex-col h-full">
      {headerEl && <div style={{ marginBottom: 4 * headerScale }}>{headerEl}</div>}
      <div className="flex-1 min-h-0"><ProductionView site={site} scale={mainScale} /></div>
    </div>
  );
  ...
  if (layout === 'consumption') return (
    <div ref={boxRef} className="aura-widget-row flex flex-col h-full">
      {headerEl && <div style={{ marginBottom: 4 * headerScale }}>{headerEl}</div>}
      <div className="flex-1 min-h-0"><ConsumptionView site={site} scale={mainScale} /></div>
    </div>
  );
  ...
  if (layout === 'compact') {
    return (
      <div ref={boxRef} className="aura-widget-row flex flex-col h-full justify-center px-1" style={{ gap: 6 * mainScale }}>
        {headerEl}
  ...
  return (
    <div ref={boxRef} className="aura-widget-row aura-scroll flex flex-col h-full overflow-auto" style={{ gap: 8 * lpScale }}>
      ...
      {site.tariffGrid > 0 && (
        <div className="text-right shrink-0" style={{ color: 'var(--text-secondary)', fontSize: 10 * tariffScale }}>
          {site.tariffGrid.toFixed(4)} €/kWh
        </div>
      )}
    </div>
  ...
  return (
    <>
      ...
      <div className="pt-2 mt-1 border-t" style={{ borderColor: 'var(--app-border)' }}>
        <div className="text-[11px] font-semibold mb-2" style={{ color: 'var(--text-primary)' }}>Größe & Skalierung</div>

        <div className="flex items-center justify-between mb-2">
          <label className="text-[11px]" style={{ color: 'var(--text-secondary)' }}>
            Auto-Skalierung <span style={{ opacity: 0.6 }}>(mit Widget-Breite)</span>
          </label>
          <button onClick={() => set({ autoScale: !autoScale })}
            className="relative w-9 h-5 rounded-full transition-colors"
        ...
        <div className="space-y-2">
          ...
          <ScaleSlider label="Haupt-Bereich (Akku/Erzeugung/Verbrauch/Kompakt)" k="mainScale" val={mainScale} />
