...
function freshTrigger(kind: TimerTrigger['kind']): TimerTrigger {
    ...
    const isoEnd = new Date(today.getTime() + 60 * 60_000 - today.getTimezoneOffset() * 60_000)
        .toISOString()
        .slice(0, 16);
            ...
            >
                ...
                <div className="flex-1 overflow-y-auto px-4 py-3 space-y-3">
                    ...
                    <div className="flex items-center gap-2">
                        <button
                            onClick={() => patch({ enabled: !event.enabled })}
                        ...
                        <div className="flex gap-1 mb-2">
                            {TRIGGER_TABS.map(({ id, label, Icon }) => {
                                ...
                                return (
                                    <button
                                        key={id}
                                        onClick={() => patch({ trigger: freshTrigger(id) })}
                        ...
                        {t.kind === 'astro' && (
                            <div className="flex gap-1.5">
                                <select
                                    value={t.event}
                                    onChange={(e) =>
                                        patch({ trigger: { ...t, event: e.target.value as TimerAstroEvent } })
                                    }
                                ...
                                <div className="flex items-center gap-1 shrink-0">
                                    <input
                                        ...
                                        value={t.offsetMin}
                                        onChange={(e) =>
                                            patch({ trigger: { ...t, offsetMin: Number(e.target.value) || 0 } })
                                        }
