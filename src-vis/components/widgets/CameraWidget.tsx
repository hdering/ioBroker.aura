...
function InfoCell({ slot, value, transparent }: { slot: CameraSlot; value: unknown; transparent?: boolean }) {
    ...
    const Lbl = label ? (
        <span className="text-[9px] truncate max-w-full text-center" style={sec}>
            {label}
        </span>
    ) : null;
...
function InfoRow({ slot, value, transparent }: { slot: CameraSlot; value: unknown; transparent?: boolean }) {
    ...
    return (
        ...
        >
            ...
            <span className="font-medium tabular-nums" style={valStyle}>
                {display}
            </span>
        </div>
    );
...
function StreamView(p: StreamViewProps) {
    ...
    const countdownColor =
        sLeft != null ? (sLeft <= 10 ? '#ef4444' : sLeft <= 60 ? '#f59e0b' : 'rgba(255,255,255,0.7)') : null;
    ...
    return (
        <div className="relative h-full w-full overflow-hidden">
            ...
            {sLeft != null && !hasError && (
                <div
                    className="absolute bottom-1 left-1 px-1.5 py-0.5 rounded text-[9px] font-mono flex items-center gap-0.5"
            ...
            {p.showTimestamp && p.lastRefresh && !hasError && p.mode === 'img' && (
                ...
                >
                    {p.lastRefresh.toLocaleTimeString('de-DE', {
                        hour: '2-digit',
                        minute: '2-digit',
                        second: '2-digit',
                    })}
            ...
            {p.editMode && (
                <div
                    ...
                    style={{
                        background:
                            p.mode === 'iframe' ? '#3b82f6' : p.refreshInterval === 0 ? '#ef4444' : 'rgba(0,0,0,0.6)',
                        color: '#fff',
                    }}
                >
...
function StreamCell({
    ...
    if (needsWake && wakeUpMode === 'onClick' && !waking && !streamReady) {
        ...
        const label =
            stopReason === 'timeout' ? 'Stream beendet' : stopReason === 'error' ? 'Verbindung verloren' : null;
        return (
            <div
                ref={sectionRef}
            ...
            >
                <div
                    ...
                    style={{
                        background: isReactivate ? 'rgba(239,68,68,0.15)' : 'var(--accent)',
                        opacity: isReactivate ? 1 : 0.85,
                        border: isReactivate ? '1.5px solid #ef4444' : 'none',
                    }}
                >
                    {isReactivate ? <RefreshCw size={16} color="#ef4444" /> : <Camera size={18} color="#fff" />}
                </div>
                {label && (
    ...
    if (needsWake && wakeUpMode === 'onView' && !waking && !streamReady) {
        return (
            <div ref={sectionRef} className="flex items-center justify-center h-full" style={{ background: wakeBg }}>
    ...
    if (layout === 'minimal') {
        ...
        if (needsWake && wakeUpMode === 'onView' && !waking && !streamReady) {
            return (
                <>
                    <div
                        ref={containerRef}
                        className="aura-widget-row flex flex-col items-center justify-center h-full gap-2 rounded-[inherit]"
        ...
        return (
            <>
                <div
                    ref={containerRef}
                    className="aura-widget-row aura-widget-value h-full w-full overflow-hidden rounded-[inherit]"
                >
                    <StreamView {...svProps} />
                </div>
                {portal}
    ...
    if (layout === 'default') {
        ...
        return (
            <>
                <div
                    ref={containerRef}
                    className="aura-widget-row h-full w-full overflow-hidden rounded-[inherit] flex flex-col"
    ...
    if (cameraTemplate === 'stream-full') {
        return (
            <>
                <div
                    ref={containerRef}
                    className="aura-widget-row relative h-full w-full overflow-hidden rounded-[inherit]"
                >
                    <StreamCell {...scProps} />
                    <div
                        className="absolute bottom-0 left-0 right-0 flex gap-1 p-1.5 flex-wrap justify-start items-end pointer-events-none"
                        ...
                        {customSlots.map((slot, i) => {
                            ...
                            switch (slot.type) {
                                ...
                                case 'armed':
                                    ...
                                    display = bool ? slot.trueLabel || 'Scharf' : slot.falseLabel || 'Aus';
                                    break;
                                case 'motion':
            ...
            >
                ...
                {Array.from({ length: emptyCount }, (_, i) => (
                    <div
                        key={`pad-${i}`}
                        style={{
                            gridArea: `slot${filledSlots.length + i}`,
                            background: transparent ? 'transparent' : 'var(--app-bg)',
                            borderRadius: '4px',
                        }}
                    />
