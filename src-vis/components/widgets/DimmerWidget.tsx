...
export function DimmerWidget({ config }: WidgetProps) {
    ...
    if (layout === 'compact') {
        return (
            <div
                className="aura-widget-row flex flex-col justify-center h-full gap-1.5"
    ...
    if (layout === 'minimal') {
        return (
            <div
                className="aura-widget-row flex flex-col items-center justify-center h-full gap-3"
                style={{ position: 'relative' }}
            >
                {showValue && (
                    <span
                ...
                {showSlider && (barStyle ? <div style={{ height: 40, width: '100%' }}>{barTrack}</div> : slider)}
                {toggleBtn}
                <StatusBadges config={config} />
            </div>
        );
            ...
            <div className="space-y-2">
                {showValue && (
                    <div className="flex justify-between items-center">
                        <span className="aura-widget-value text-xl font-bold" style={{ color: valueColor }}>
                            {level}%
                        </span>
                        {showToggle ? (
                            toggleBtn
