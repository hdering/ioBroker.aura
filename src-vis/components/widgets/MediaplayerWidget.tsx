...
function CoverImage({ src, Icon, iconSize }: { src: string; Icon: React.ElementType | null; iconSize: number }) {
    if (src) {
        return <img src={src} alt="" className="w-full h-full rounded-xl object-cover" />;
    }
    return (
        <div
...
function ChipRow({ chips, onTrigger }: { chips: MediaChip[]; onTrigger: (chip: MediaChip) => void }) {
    return (
...
function ProgressBar({ pct, progressStr, durationStr }: { pct: number; progressStr?: string; durationStr?: string }) {
...
export function MediaplayerWidget({ config }: WidgetProps) {
    ...
    const volPct =
        typeof rawVol === 'number' ? Math.min(100, Math.max(0, Math.round(((rawVol - volMin) / volRange) * 100))) : 0;
            ...
            <div className="aura-widget-row flex flex-col gap-3 w-full">
                ...
                <div className="shrink-0 space-y-0.5 text-center min-w-0">
                    ...
                    {showSource && sourceStr && (
                        <p
                            className="flex items-center justify-center gap-1 text-xs truncate"
                ...
                {(showVolume || showMute) && (
                    <div className="shrink-0 space-y-1.5 px-1">
                        <div className="flex items-center gap-2">
                            {showMute && <IconBtn icon={isActuallyMuted ? VolumeX : Volume2} onClick={handleMute} />}
                            {showVolume && <VolumeSlider pct={volPct} step={volStep} onChange={writeVol} />}
                            {showVolume && (
                                <span
                                    className="shrink-0 text-[11px] tabular-nums"
                                    style={{ color: 'var(--text-secondary)', minWidth: '28px', textAlign: 'right' }}
                                >
                                    {volPct} %
                                </span>
                            )}
                ...
                {showChips && (
                    <div className="shrink-0">
                        <ChipRow chips={chips} onTrigger={handleChip} />
                    </div>
                )}
                            ...
                            {showVolume && (
                                <>
                                    ...
                                    <span
                                        ...
                                        style={{ color: 'var(--text-secondary)', minWidth: '24px' }}
                                    >
                                        {volPct}%
                                    </span>
                                </>
