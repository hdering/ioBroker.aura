...
export function TrashWidget({ config }: WidgetProps) {
    ...
    const bins: TrashBin[] = (opts.bins as TrashBin[]) ?? [];
    const showTitle = opts.showTitle !== false;
    const showIcon = opts.showIcon !== false;
    const iconSize = (opts.iconSize as number) || 20;
    const titleAlign = (opts.titleAlign as string) ?? 'left';
    const WidgetIcon = getWidgetIcon(opts.icon as string | undefined, Truck);
...
const DEFAULT_COLORS = ['#6b7280', '#3b82f6', '#22c55e', '#f59e0b', '#ef4444', '#8b5cf6', '#06b6d4', '#84cc16'];
        ...
        <>
            ...
            {pickerBinId && (
                <DatapointPicker
                    ...
                    onSelect={(dp) => {
                        updateBin(pickerBinId, { datapoint: dp });
                        setPickerBinId(null);
                    }}
                        ...
                        >
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <div className="flex gap-1 shrink-0">
                                    ...
                                    <button
                                        onClick={() => moveBin(bin.id, 1)}
                                        disabled={idx === bins.length - 1}
                                        className="text-[11px] w-5 h-5 flex items-center justify-center rounded"
                                        style={{
                                            color: 'var(--text-secondary)',
                                            background: 'var(--widget-bg)',
                                            border: '1px solid var(--app-border)',
                                            opacity: idx === bins.length - 1 ? 0.3 : 1,
                                        }}
                                    >
                                        ↓
                                    </button>
                                    ...
                                    {TRASH_ICON_OPTIONS.map((opt) => {
                                        ...
                                        return (
                                            ...
                                            >
                                                <Ic
                                                    size={15}
                                                    color={selected ? bin.color : 'var(--text-secondary)'}
                                                    strokeWidth={1.5}
                                                />
                                            </button>
                                        );
                            ...
                            <div>
                                <label className="text-[11px] mb-1 block" style={{ color: 'var(--text-secondary)' }}>
                                    Icon ausblenden wenn Wert ist
                                </label>
                                <select
                                    value={bin.hideWhen}
                                    onChange={(e) =>
                                        updateBin(bin.id, { hideWhen: e.target.value as TrashBin['hideWhen'] })
                                    }
                                    className={iCls}
