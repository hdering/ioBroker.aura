...
export function KnobWidget({ config }: WidgetProps) {
    ...
    const renderCircleLayout = () => {
        ...
        const labelDecimals =
            labelStep > 0 && labelStep < 1 ? Math.min(3, Math.max(0, -Math.floor(Math.log10(labelStep)))) : 0;
        ...
        const renderEndlessPointer = () => {
            ...
            if (pointerStyle === 'arrow') {
                ...
                return (
                    <polygon
                        points={`${tipP.x},${tipP.y} ${bL.x},${bL.y} ${bR.x},${bR.y}`}
                        fill="#1a1a1a"
            ...
            <>
                ...
                {showValue && (
                    <text
                        ...
                        fill="#ffffff"
                        style={
                            { pointerEvents: 'none', textShadow: '0 1px 2px rgba(0,0,0,0.6)' } as React.CSSProperties
                        }
                    >
                        {valueStr}
                    </text>
    ...
    const renderScaleLayout = () => {
        ...
        const renderScalePointer = () => {
            ...
            if (pointerStyle === 'circle') {
                ...
                return (
                    <circle cx={p.x} cy={p.y} r={arcStroke / 2 + 1.5} fill={arcColor} stroke="#fff" strokeWidth={1.5} />
                );
            }
            ...
            return <polygon points={`${v1.x},${v1.y} ${v2.x},${v2.y} ${v3.x},${v3.y}`} fill={arcColor} />;
        };
        ...
        return (
            <>
                ...
                {/* Thick progress arc */}
                <path
                    d={describeArc(cx, cy, arcR, startAngle, currentAngle)}
                    fill="none"
                    stroke={arcColor}
                    strokeWidth={arcStroke}
                    strokeLinecap="butt"
    ...
    const renderBoundedLayout = () => {
        ...
        return (
            <>
                ...
                />
                <path
                    d={describeArc(cx, cy, trackR, startAngle, currentAngle)}
                    fill="none"
                    stroke={color}
                    strokeWidth={trackStroke}
                    strokeLinecap="round"
                ...
                <circle
                    ...
                    strokeWidth={1}
                />
                <circle cx={cx} cy={cy} r={bodyR} fill={`url(#${innerGradId})`} style={{ pointerEvents: 'none' }} />
    ...
    if (isCustom) {
        ...
        if (
            ...
            (isLegacyKnobAxis(customGrid.colSizes) || isLegacyKnobAxis(customGrid.rowSizes))
        ) {
            customGrid = {
                ...customGrid,
