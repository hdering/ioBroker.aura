...
function weekdaysText(wd: TimerWeekday[]): string {
    ...
    const isWeekdaysOnly =
        ['mon', 'tue', 'wed', 'thu', 'fri'].every((d) => set.has(d as TimerWeekday)) &&
        !set.has('sat') &&
        !set.has('sun');
...
export function TimerWidget({ config, editMode, onConfigChange }: WidgetProps) {
    ...
    const masterSwitch = (
        <button
            onClick={interactive ? toggleMaster : undefined}
            disabled={!interactive}
            className="nodrag relative w-10 h-5 rounded-full transition-colors shrink-0"
            style={{
                background: masterEnabled ? 'var(--accent-green)' : 'var(--app-border)',
            ...
            title={
                interactive
                    ? masterEnabled
                        ? 'Master aktiv'
                        : 'Master aus — Ereignisse bleiben gespeichert'
                    : 'Im Bearbeitungsmodus deaktiviert'
            }
        >
            <span
        ...
        <div className="flex flex-col gap-1 w-full">
            {visibleEvents.map((ev) => {
                ...
                return (
                    ...
                    >
                        <button
                            onClick={interactive ? () => toggleEvent(ev.id) : undefined}
                            disabled={!interactive}
                            className="nodrag w-3 h-3 rounded-full shrink-0 transition-colors"
                            style={{
                                background: isLive ? 'var(--accent-green)' : 'var(--app-border)',
                                cursor: interactive ? 'pointer' : 'default',
                            }}
                            title={
                                interactive
                                    ? ev.enabled
                                        ? 'Aktiv – Klick deaktiviert'
                                        : 'Inaktiv – Klick aktiviert'
                                    : ''
                            }
                        />
                        <button
                        ...
                        >
                            <span
                                ...
                                style={{ color: 'var(--text-secondary)', minWidth: 48 }}
                            >
                                {ev.trigger.kind === 'once' || ev.trigger.kind === 'range'
                                    ? '—'
    ...
    return (
        <div className={`aura-widget-row flex flex-col h-full gap-1.5 ${posClass}`}>
            ...
            <p
                ...
                style={{ color: 'var(--text-secondary)' }}
                title={
                    hasTarget ? `Ziel: ${targetDp} ← ${targetValue ?? 'true'}` : 'Kein Ziel-Datenpunkt — bitte Admin'
                }
            >
                {!hasTarget
                    ? '⚠ Admin: Ziel-DP fehlt'
            ...
            {!showEvents && showAdd && <div className="mt-auto">{addButton}</div>}
