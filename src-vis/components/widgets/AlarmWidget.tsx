...
import {
  ShieldAlert, ShieldCheck, ShieldOff, Moon, Home, AlertTriangle, Bell,
  Lock, Activity, Clock as ClockIcon, Ban, KeyRound, Send,
} from 'lucide-react';
...
const DEFAULT_DATA: AlarmData = {
  stateCode: 0, stateText: '', activated: false, activatedWarnings: false, sleep: false,
  sharpInside: false, burglar: false, silent: false, siren: false, enableable: true,
  activationCountdown: 0, silentCountdown: 0,
  alarmCircuitList: '', notificationCircuitList: '', sharpInsideCircuitList: '',
  wrongPassword: false, logLast: '', logToday: '',
};
...
function ModeButton({
  active, color, label, Icon, onClick, disabled, fontSize, iconSize,
}: {
  active: boolean; color: string; label: string;
  Icon: React.ComponentType<{ size?: number; color?: string }>;
  onClick: () => void; disabled?: boolean; fontSize: number; iconSize: number;
}) {
  return (
    <button
      onClick={onClick}
    ...
    >
      ...
      <span className="truncate" style={{ fontSize, lineHeight: 1.1 }}>{label}</span>
    </button>
  );
}
...
export function AlarmWidget({ config }: WidgetProps) {
  ...
  useEffect(() => { if (!data.burglar && !data.silent) setPinOpen(false); }, [data.burglar, data.silent]);
    ...
    <div className="aura-widget-row w-full h-full flex flex-col overflow-hidden" style={{ gap }}>
      ...
      {(showTitle || showIcon) && (
        <div className="flex items-center gap-2 shrink-0">
          {showIcon && <WidgetIcon size={iconSize} style={{ color: triggered ? '#ef4444' : 'var(--accent)' }} className="aura-widget-icon shrink-0" />}
          {showTitle && (
            <p
      ...
      {showHeader && (
        ...
        >
          <div className="flex items-center min-w-0" style={{ gap: 6 * sizeScale }}>
            <StatePill code={data.stateCode} label={t(stateInfo.labelKey) || data.stateText} fontSize={fsTiny} />
            {data.sleep && (
              <span className="inline-flex items-center gap-1" style={{ color: '#6366f1', fontSize: fsTiny }}>
      ...
      {triggered && (pinOpen || requirePin) ? (
        <div className="flex items-center rounded-xl shrink-0" style={{
          padding: 8 * sizeScale, background: 'var(--app-bg)', border: '1px solid var(--app-border)', gap: 6 * sizeScale,
        }}>
          ...
          <input
            ...
            style={{ background: 'var(--app-surface)', color: 'var(--text-primary)', border: '1px solid var(--app-border)', fontSize: fsBase }}
          ...
          <button
            ...
            style={{
              ...
              background: '#ef4444', color: '#fff', border: '1px solid #ef4444', fontSize: fsSmall,
            }}
          >
        ...
        showModes && (
          <div className="grid shrink-0" style={{
            gridTemplateColumns: compactMode ? 'repeat(2, minmax(0, 1fr))' : 'repeat(4, minmax(0, 1fr))',
            gap: 6 * sizeScale,
          }}>
            <ModeButton
          ...
          {showDelay && (
            <button
              ...
              style={{
                ...
                border: '1px solid var(--app-border)', fontSize: fsSmall,
              }}
            >
          ...
          {showPanic && (
            <button
              ...
              style={{
                ...
                border: '1px solid #ef444455', fontSize: fsSmall, fontWeight: 600,
              }}
            >
      ...
      {showCountdown && (data.activationCountdown > 0 || data.silentCountdown > 0) && (
        <div className="flex items-center rounded-lg shrink-0" style={{
          padding: `${4 * sizeScale}px ${8 * sizeScale}px`,
          background: '#eab30822', border: '1px solid #eab30855', color: '#eab308',
          fontSize: fsSmall, gap: 6 * sizeScale,
        }}>
          <ClockIcon size={Math.round(13 * sizeScale)} />
          {data.activationCountdown > 0
            ? <>{t('alarm.activatingIn') as string} <strong>{fmtCountdown(data.activationCountdown)}</strong></>
            : <>{t('alarm.silentIn') as string} <strong>{fmtCountdown(data.silentCountdown)}</strong></>}
        </div>
      )}

      {showCircuits && data.alarmCircuitList && (
        <div className="flex items-start rounded-lg shrink-0" style={{
          padding: `${4 * sizeScale}px ${8 * sizeScale}px`,
          ...
          <span className="flex-1">{t('alarm.openCircuits') as string}: {data.alarmCircuitList}</span>
          <button
            onClick={doQuit}
            className="rounded inline-flex items-center gap-1"
            style={{
              padding: `${2 * sizeScale}px ${6 * sizeScale}px`,
              fontSize: fsTiny, color: '#ef4444', border: '1px solid #ef444499', background: 'transparent',
            }}
      ...
      {showCircuits && !data.alarmCircuitList && data.notificationCircuitList && (
        <div className="flex items-center rounded-lg shrink-0" style={{
          ...
          background: 'var(--app-bg)', border: '1px dashed var(--app-border)', color: 'var(--text-secondary)',
          fontSize: fsTiny, gap: 6 * sizeScale,
        }}>
      ...
      {showLog && logEntries.length > 0 && (
        <div className="flex flex-col rounded-lg overflow-hidden min-h-0" style={{
          background: 'var(--app-bg)', border: '1px solid var(--app-border)',
          padding: `${4 * sizeScale}px ${8 * sizeScale}px`,
          gap: 2 * sizeScale,
        }}>
          <div className="flex items-center gap-1 shrink-0" style={{ fontSize: fsTiny, color: 'var(--text-secondary)' }}>
          ...
          <div className="flex flex-col overflow-auto min-h-0" style={{ gap: 1 * sizeScale }}>
            {logEntries.map((entry, i) => (
              <div key={i} className="flex" style={{ gap: 6 * sizeScale, fontSize: fsTiny }}>
                <span className="tabular-nums shrink-0" style={{ color: 'var(--text-secondary)', minWidth: 36 * sizeScale }}>{entry.time}</span>
                <span className="flex-1 min-w-0 truncate" style={{ color: 'var(--text-primary)' }}>{entry.text}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      {confirmAction && (
        ...
        >
          <div className="rounded-xl p-4 flex flex-col items-center"
            style={{ background: 'var(--app-surface)', border: '1px solid var(--app-border)', gap: 8, minWidth: 200 }}
          ...
          >
            ...
            <div className="flex gap-2">
              <button onClick={() => setConfirmAction(null)}
                ...
                style={{ background: 'var(--app-bg)', color: 'var(--text-primary)', border: '1px solid var(--app-border)', fontSize: fsSmall }}>
                {t('alarm.cancel') as string}
              </button>
              <button onClick={() => { confirmAction(); setConfirmAction(null); }}
                className="rounded-lg px-3 py-1 font-semibold"
                style={{ background: '#ef4444', color: '#fff', border: '1px solid #ef4444', fontSize: fsSmall }}>
...
function ZonesRow({ prefix, sizeScale }: { prefix: string; sizeScale: number }) {
  ...
  return (
    <div className="flex shrink-0" style={{ gap: 6 * sizeScale }}>
      {zones.map((z) => (
        <ZoneTile key={z.key} prefix={prefix} zoneKey={z.key} label={z.label} sizeScale={sizeScale} setState={setState} />
      ))}
    </div>
  );
}
...
function ZoneTile({
  prefix, zoneKey, label, sizeScale, setState,
}: {
  prefix: string; zoneKey: string; label: string; sizeScale: number;
  ...
  return (
    ...
    >
      <span className="flex items-center gap-1" style={{ color, fontSize: fsLabel, fontWeight: 600 }}>
        <span style={{ width: 6, height: 6, borderRadius: '50%', background: color, opacity: isOn ? 1 : 0.35 }} />
        {label}
      </span>
...
export function AlarmConfig({
  ...
  return (
    <>
      <div>
        <label className="text-[11px] mb-1 block" style={{ color: 'var(--text-secondary)' }}>{t('alarm.cfg.prefix') as string}</label>
        <input type="text" value={prefix}
          onChange={(e) => set({ alarmPrefix: e.target.value || 'alarm.0' })}
          placeholder="alarm.0" className={inputCls + ' font-mono'} style={inputSty} />
      </div>
      ...
      <div className="pt-1 mt-1 border-t" style={{ borderColor: 'var(--app-border)' }}>
        ...
        <div className="space-y-2">
          ...
          <Toggle label={t('alarm.cfg.showPanic')     as string} k="showPanic"     def={false} hint={t('alarm.cfg.showPanic.hint') as string} />
      ...
      <div className="pt-1 mt-1 border-t" style={{ borderColor: 'var(--app-border)' }}>
        ...
        <div className="space-y-2">
          <Toggle label={t('alarm.cfg.requirePin')    as string} k="requirePinForDisarm" def={false} hint={t('alarm.cfg.requirePin.hint') as string} />
          <Toggle label={t('alarm.cfg.panicConfirm')  as string} k="panicConfirm"        def={true} />
          <Toggle label={t('alarm.cfg.compactMode')   as string} k="compactMode"         def={false} hint={t('alarm.cfg.compactMode.hint') as string} />
        </div>
      </div>
      ...
      <div>
        ...
        <input type="range" min={0} max={20} step={1} value={logLines}
      ...
      <div>
        ...
        <input type="range" min={0.5} max={2.5} step={0.05} value={sizeScale}
