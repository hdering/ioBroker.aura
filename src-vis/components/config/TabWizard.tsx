...
export function TabWizard({ onAdd, onClose }: TabWizardProps) {
    ...
    const selectedDps = useMemo(() => datapoints.filter((dp) => checkedIds.has(dp.id)), [datapoints, checkedIds]);
                    ...
                    {step === 'intro' && (
                        <div className="space-y-5">
                            <p className="text-sm" style={{ color: 'var(--text-secondary)' }}>
                                Der Tab-Wizard erstellt einen neuen Tab und befüllt ihn automatisch mit passenden
                                Widgets.
                            </p>
                                    ...
                                    <button
                                        onClick={() =>
                                            setCheckedIds((prev) => {
                                                ...
                                                shown
                                                    .filter((dp) => isRelevantDp(dp.role, dp.type))
                                                    .forEach((dp) => next.add(dp.id));
                                                return next;
                                            })
                                        }
                    ...
                    {step === 'review' && (
                        <div className="space-y-2">
                            <p className="text-xs mb-3" style={{ color: 'var(--text-secondary)' }}>
                                {selectedDps.length} Datenpunkt{selectedDps.length !== 1 ? 'e' : ''} – Widget-Typ prüfen
                                und ggf. ändern.
                            </p>
                            {selectedDps.map((dp) => {
