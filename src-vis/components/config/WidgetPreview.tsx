...
const MOCK = Object.fromEntries(Object.entries(WIDGET_BY_TYPE).map(([t, m]) => [t, m.mock])) as Record<
    WidgetType,
    { t: string; v: string; u?: string; sub?: string }
>;
...
function Toggle({ on }: { on?: boolean }) {
    return (
        <div
            ...
            style={{ background: on !== false ? 'var(--accent-green)' : 'var(--app-border)' }}
        >
            <span
...
function MockContent({ type, layout, title }: { type: WidgetType; layout: WidgetLayout; title: string }) {
    ...
    if (type === 'calendar') {
        const mockEvents = [
            ...
            {
                label: tr('preview.cal.dentist'),
                date: tr('calendar.tomorrowAt', { time: '14:30' }),
                dot: 'var(--accent-green)',
            },
            {
                label: tr('preview.cal.birthday'),
                date: `${tr('cal.day.3')}, 9. ${tr('cal.month.3')}`,
                ...
                <div className="flex items-center gap-2 h-full">
                    ...
                    <div>
                        ...
                        <p className="text-[10px]" style={{ color: 'var(--accent)' }}>
                            {tr('calendar.todayAt', { time: '10:00' })}
                        </p>
                    </div>
                </div>
            );
                ...
                <div className="flex flex-col h-full gap-0.5">
                    ...
                    {mockEvents.map((ev, i) => (
                        <div key={i} className="flex items-center gap-1.5">
                            ...
                            <span className="text-[8px] w-8 truncate shrink-0" style={{ color: ev.dot }}>
                                Cal {i + 1}
                            </span>
                            <p
                                className="flex-1 text-[9px] font-medium truncate"
                                style={{ color: 'var(--text-primary)' }}
                            >
                                {ev.label}
                            </p>
                            <p className="text-[8px] shrink-0" style={{ color: 'var(--text-secondary)' }}>
                ...
                <div className="flex gap-1 pt-1" style={{ borderTop: '1px solid var(--app-border)' }}>
                    {[
                        ...
                        >
                            ...
                            <span style={{ color: 'var(--text-primary)', fontWeight: 'bold' }}>{tmp}</span>
                        </div>
                    ))}
                </div>
            </div>
        ...
        return (
            <div className="flex flex-col h-full gap-1.5">
                ...
                <div className="flex gap-1.5 flex-1 min-h-0">
                    ...
                    <div
                        className="flex flex-col gap-0.5 justify-center text-[11px]"
        ...
        return (
            <div className="flex flex-col items-center justify-center h-full gap-1">
                <svg viewBox="0 0 200 120" style={{ width: 120, height: 72 }}>
                    ...
                    />
                    <path
                        d="M 20 100 A 80 80 0 0 1 131.13 33.57"
                        fill="none"
                        stroke="var(--accent)"
                        strokeWidth={12}
                        strokeLinecap="round"
                    ...
                    <text x={100} y={110} textAnchor="middle" fontSize={10} fill="var(--text-secondary)">
                        {m.u}
                    </text>
                </svg>
                {t && (
        ...
        return (
            <div className="flex flex-col items-center justify-center h-full gap-1">
                <svg viewBox="0 0 200 200" style={{ width: 90, height: 90 }}>
                    ...
                    <path
                        d="M 50.5 149.5 A 70 70 0 1 1 145.3 153.2"
                        fill="none"
                        stroke="var(--accent)"
                        strokeWidth={12}
                        strokeLinecap="round"
    ...
    if (type === 'camera') {
        return (
            <div
                ...
                style={{ background: 'var(--app-bg)', borderRadius: 4 }}
            >
        ...
        return (
            <div className="flex flex-col h-full gap-1.5">
                ...
                <p className="text-[11px] font-bold" style={{ color: 'var(--text-primary)' }}>
                    {m.v}
                    {m.u && <span style={{ color: 'var(--text-secondary)', fontWeight: 400 }}> {m.u}</span>}
                </p>
            </div>
        );
    }
    ...
    if (layout === 'card') {
        if (type === 'switch') {
            return (
                <div
                    className="flex flex-col items-center justify-center h-full gap-2 rounded-md"
                    style={{ background: 'color-mix(in srgb, var(--accent-green) 20%, transparent)' }}
                >
    ...
    if (layout === 'compact') {
        return (
            <div className="flex items-center gap-2 h-full">
                ...
                {type === 'dimmer' && (
                    <div
                        className="w-14 h-2 rounded-full overflow-hidden shrink-0"
            ...
            <div>
                {type === 'switch' ? (
                    ...
                    <div>
                        ...
                        <div
                            className="w-full h-2 mt-1 rounded-full overflow-hidden"
                            style={{ background: 'var(--app-border)' }}
                        >
                            <div
                                className="h-full rounded-full"
                                style={{ width: '75%', background: 'var(--accent)' }}
                            />
                        </div>
                    </div>
                ) : (
...
export function WidgetPreview({
    ...
    title,
    scale: scaleProp,
}: WidgetPreviewProps & { scale?: number }) {
