...
const WIDGET_LABELS = Object.fromEntries(Object.entries(WIDGET_BY_TYPE).map(([t, m]) => [t, m.label])) as Record<
    WidgetType,
    string
>;
...
export function DeviceWizard({ onAdd, onClose }: DeviceWizardProps) {
        ...
        setSelections((prev) => {
            ...
            if (next.has(state.id)) next.delete(state.id);
                ...
                next.set(state.id, {
                    state,
                    device,
                    widgetType: state.suggestedWidget,
    ...
    const handleAdd = () => {
        onAdd(
            Array.from(selections.values()).map((sel, i) => ({
                id: `w-${Date.now()}-${i}`,
                type: sel.widgetType,
                title: sel.title,
                ...
                gridPos: {
                    x: (i * 2) % 12,
                    y: 9999,
                    w: sel.widgetType === 'chart' ? 4 : 2,
                    h: sel.widgetType === 'chart' ? 3 : 2,
                },
                options: {
                ...
                {!loaded ? (
                    <div className="flex flex-col items-center justify-center flex-1 gap-4 py-12">
                        {loading ? (
                            ...
                            <>
                                ...
                                <button
                                    onClick={load}
                                    className="px-6 py-2 text-white rounded-lg font-medium hover:opacity-80"
                    ...
                    <>
                        ...
                        <div className="px-6 py-3 space-y-2" style={{ borderBottom: '1px solid var(--app-border)' }}>
                            <div className="flex gap-3">
                                <input
                                    ...
                                    placeholder={t('wizard.device.search')}
                                ...
                                <span
                                    className="text-sm self-center whitespace-nowrap"
                                    style={{ color: 'var(--text-secondary)' }}
                                >
                                    {filtered.length} {t('wizard.device.devices')}
                                </span>
                            </div>
                        ...
                        <div className="aura-scroll flex-1 overflow-y-auto px-6 py-3 space-y-2">
                            {filtered.map((device) => {
                                const cnt = Array.from(selections.values()).filter(
                                    (s) => s.device.id === device.id,
                                ).length;
                                return (
                                    <div
                                        key={device.id}
                                        ...
                                        >
                                            ...
                                            <div className="flex items-center gap-3">
                                                ...
                                                <span className="text-xs" style={{ color: 'var(--text-secondary)' }}>
                                                    {device.states.length} DP {expandedDevice === device.id ? '▲' : '▼'}
                                                </span>
                                            </div>
                                        </button>
                                            ...
                                            device.states.map((state) => {
                                                ...
                                                return (
                                                    <div key={state.id}>
                                                            ...
                                                            <div className="flex items-center gap-2 shrink-0">
                                                                ...
                                                                {sel ? (
                                                                    <select
                                                                        ...
                                                                        style={inputStyle}
                                                                    >
                                                                        {Object.entries(WIDGET_LABELS).map(([t, l]) => (
                                                                    ...
                                                                    <span
                                                                        ...
                                                                        style={{ color: 'var(--text-secondary)' }}
                                                                    >
                                                                        {WIDGET_LABELS[state.suggestedWidget]}
                                                                    </span>
                                                                )}
                                                            </div>
                                                        </div>
                                                        {sel && (
                                                            <div
                                                                className="px-4 pb-3 pt-2 flex gap-3 items-start"
                                                                style={{
                                                            ...
                                                            >
                                                                <div className="flex-1 space-y-2 min-w-0">
                                                                    ...
                                                                    {sel.widgetType === 'thermostat' && (
                                                                        <input
                                                                            value={sel.actualDatapoint ?? ''}
                                                                            onChange={(e) =>
                                                                                updateSel(state.id, {
                                                                                    actualDatapoint: e.target.value,
                                                                                })
                                                                            }
                                                                            placeholder={t('wizard.device.tempDp')}
                                                                            className="w-full text-xs rounded px-2 py-1.5 focus:outline-none"
                                                                            style={inputStyle}
                                                                        />
                                                                    )}
                        ...
                        >
                            <span className="text-sm" style={{ color: 'var(--text-secondary)' }}>
                                {selections.size === 0
                                    ? t('wizard.device.noneSelected')
                                    : `${selections.size} ${selections.size !== 1 ? t('wizard.device.widgets') : t('wizard.device.widget')}`}
                            </span>
