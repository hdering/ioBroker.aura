...
export function TimerConfig({ config, onConfigChange }: Props) {
  ...
  const currentPickerValue =
      pickerTarget === 'target'   ? targetDp
    : pickerTarget === 'holidays' ? holidaysDp
    : pickerTarget === 'vacation' ? vacationDp
    : '';
    ...
    <>
      ...
      <div className="space-y-3">
        ...
        <div className="rounded-xl p-2 space-y-2"
          style={{ background: 'var(--app-bg)', border: '1px solid var(--app-border)' }}>
          <p className="text-[11px] font-semibold" style={{ color: 'var(--text-primary)' }}>Ziel-Aktion</p>
          <p className={hintCls} style={hintStyle}>
            Was das Widget tut, wenn ein Ereignis auslöst. Nur hier (Admin) konfigurierbar — der Anwender legt nur Zeiten fest.
          </p>

          <div>
            <label className={labelCls} style={labelStyle}>Ziel-Datenpunkt</label>
            <div className="flex gap-1">
              <input type="text" value={targetDp}
              ...
              <button type="button" onClick={() => setPickerTarget('target')}
        ...
        <div className="rounded-xl p-2 space-y-2"
          ...
          <div>
            <label className={labelCls} style={labelStyle}>Feiertage-DP</label>
            <div className="flex gap-1">
            ...
            <pre className="text-[10px] mt-1 px-2 py-1 rounded-md font-mono whitespace-pre-wrap"
              style={{ background: 'var(--app-surface)', color: 'var(--text-secondary)', border: '1px solid var(--app-border)' }}>{`Beispiel:
[
  "2026-01-01",
  "2026-04-03",
  "2026-12-25",
  "2026-12-26"
          ...
          <div>
            <label className={labelCls} style={labelStyle}>Urlaub-DP</label>
            <div className="flex gap-1">
...
[
  ...
  "2026-07-22"
          ...
          <p className={hintCls} style={hintStyle}>
            Tipp: Solche Listen können z. B. mit dem Adapter <code className="font-mono">iobroker.javascript</code> oder einer einfachen Blockly-Routine gepflegt werden.
          </p>
        </div>
      </div>
    </>
