...
export function EnumConfig({ config, onConfigChange }: Props) {
  ...
  const remove = (idx: number) =>
    setO({ entries: entries.filter((_, i) => i !== idx) });
  ...
  const importFromStates = async () => {
    ...
    try {
      ...
      const imported: EnumEntry[] = Object.entries(parsed).map(([k, v]) => ({ value: String(k), label: String(v) }));
  ...
  return (
    <>
      <div className="flex items-center gap-2">
        ...
        {importStatus && (
          <span className="text-[10px]" style={{ color: 'var(--text-secondary)' }}>{importStatus}</span>
        )}
      </div>
      ...
      <div className="space-y-1.5">
        ...
        {entries.map((entry, idx) => (
          <div key={idx} className="rounded-lg p-2 space-y-1.5" style={{ background: 'var(--app-bg)', border: '1px solid var(--app-border)' }}>
            <div className="flex items-center gap-1">
              ...
              <input
                ...
                style={{ width: '24px', height: '26px', border: '1px solid var(--app-border)', background: 'transparent' }}
              />
              {entry.color && (
                <button
              ...
              <button onClick={() => remove(idx)} className="hover:opacity-70 shrink-0" style={{ color: 'var(--accent-red, #ef4444)' }}>
                <Trash2 size={12} />
              </button>
            </div>
          </div>
