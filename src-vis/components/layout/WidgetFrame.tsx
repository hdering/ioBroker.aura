  ...
  return (
    ...
    >
      ...
      {openPanel === 'edit' && (
        ...
        >
          {/* ─── 1. Name / Titel ──────────────────────────────────────────── */}
          {!['shutter', 'switch', 'dimmer'].includes(config.type) && (<>
          ...
          <div className="space-y-2.5">
            ...
            {/* Icon picker (not for stateimage/windowcontact – icons are per-state; not for shutter/switch/dimmer – in Darstellung) */}
            {config.type !== 'stateimage' && config.type !== 'windowcontact' && !['shutter', 'switch', 'dimmer'].includes(config.type) && (() => {
            ...
            {/* Icon-Größe */}
            {!['clock', 'calendar', 'gauge', 'chart', 'echart', 'echartsPreset', 'fill', 'iframe', 'html', 'jsontable', 'image', 'camera', 'list', 'autolist', 'header', 'trash', 'trashSchedule', 'evcc', 'weather', 'group', 'shutter', 'switch', 'dimmer'].includes(config.type) && (() => {
              const o = config.options ?? {};
