...
const COMPONENT_OPTIONS: Record<string, { key: string; label: string }[]> = {
  ...
  dimmer:        [{ key: 'icon', label: 'Widget-Icon' }, { key: 'slider', label: 'Dimmer-Slider' }, { key: 'toggle', label: 'An/Aus-Schalter' }, { key: 'battery-icon', label: 'Batterie-Icon' }, { key: 'reach-icon', label: 'Erreichbarkeit-Icon' }, { key: 'status-badges', label: 'Status-Badges (alle)' }],
    ...
    <>
          ...
          <div>
            ...
            <div className="flex gap-1">
              {(['contain', 'cover', 'fill'] as const).map((fit) => {
                ...
                return (
                  <button key={fit} onClick={() => onChange({ objectFit: fit === 'contain' ? undefined : fit })}
            ...
            <div>
              ...
              <div className="flex gap-1">
                {([['toggle', 'Schiebeschalter'], ['icon', 'Icon']] as const).map(([val, lbl]) => (
                  <button key={val} onClick={() => onChange({ controlMode: val })}
          ...
          {([
            ...
            <div key={key} className="flex-1">
              ...
              <input
                ...
                onChange={(e) => onChange({ [key]: e.target.value === '' ? undefined : Number(e.target.value) } as Partial<CustomCell>)}
          ...
          <div>
            ...
            <div className="flex gap-1">
              {([['false', 'Standard'], ['true', 'Balken']] as const).map(([val, lbl]) => {
                ...
                return (
                  <button key={val} onClick={() => onChange({ barStyle: val === 'true' })}
                    className="flex-1 text-[11px] py-1.5 rounded-lg transition-colors"
                    style={{
          ...
          <div>
            ...
            <div className="flex gap-1">
              {([['horizontal', 'Horizontal'], ['vertical', 'Vertikal']] as const).map(([val, lbl]) => {
                ...
                return (
                  <button key={val} onClick={() => onChange({ orientation: val })}
                    className="flex-1 text-[11px] py-1.5 rounded-lg transition-colors"
                    style={{
                      background: active ? 'var(--accent)' : 'var(--app-bg)',
                      color:      active ? '#fff'          : 'var(--text-secondary)',
          ...
          {cell.barStyle && (
            <div>
              <div className="flex items-center justify-between mb-1">
                ...
                <span className="text-[11px] tabular-nums" style={{ color: 'var(--text-primary)' }}>{cell.barSize ?? 100}%</span>
              </div>
              <input type="range" min={10} max={100} step={5} value={cell.barSize ?? 100}
      ...
      {cell.type === 'icon' && (() => {
        ...
        const Preview = iconName
          ? getWidgetIcon(iconName, (() => null) as unknown as LucideIcon)
          : null;
        return (
          <div>
            ...
            <button
              ...
              style={{ background: 'var(--app-bg)', border: '1px solid var(--app-border)', color: 'var(--text-primary)' }}>
              {Preview
                ? <Preview size={14} style={{ flexShrink: 0, color: cell.color || 'var(--text-primary)' }} />
                : <span style={{ width: 14, height: 14, display: 'inline-block', flexShrink: 0 }} />}
              <span className="flex-1 truncate text-[11px]"
          ...
          <div>
            ...
            <div className="flex gap-1">
              {([['text', 'Text'], ['number', 'Zahl']] as const).map(([val, lbl]) => {
                ...
                return (
                  <button key={val} onClick={() => onChange({ inputMode: val })}
                    className="flex-1 text-[11px] py-1.5 rounded-lg transition-colors"
                    style={{
                      background: active ? 'var(--accent)' : 'var(--app-bg)',
                      color:      active ? '#fff'          : 'var(--text-secondary)',
        ...
        return (
          <>
            <div className="flex items-center justify-between">
              ...
              <button
                onClick={() => onChange({ timeOnly: !cell.timeOnly, showTime: !cell.timeOnly ? true : cell.showTime })}
                className="relative w-7 h-4 rounded-full transition-colors shrink-0"
      ...
      {(cell.type === 'value' || cell.type === 'dp' || cell.type === 'stepper' || cell.type === 'progress') && (
        <div>
          ...
          <div className="flex gap-1">
            ...
            />
            <button
              onClick={() => onChange({ decimals: cell.decimals === undefined ? defaultDecimals : undefined })}
              title={cell.decimals === undefined ? 'Globale Einstellung aktiv – klicken für eigenen Wert' : 'Auf globale Einstellung zurücksetzen'}
              className="text-[10px] px-2 py-1 rounded-lg shrink-0"
              style={{ background: cell.decimals === undefined ? 'var(--accent)' : 'var(--app-border)', color: cell.decimals === undefined ? '#fff' : 'var(--text-secondary)', border: 'none' }}
            >
              Global
            </button>
          </div>
          ...
          <div>
            ...
            <div className="flex gap-1">
              {(['top', 'middle', 'bottom'] as const).map((v) => {
                ...
                return (
                  <button key={v} onClick={() => onChange({ valign: v })}
                    className="flex-1 text-[10px] py-1 rounded"
                    style={{ background: active ? 'var(--accent)' : 'var(--app-bg)', color: active ? '#fff' : 'var(--text-secondary)', border: `1px solid ${active ? 'var(--accent)' : 'var(--app-border)'}` }}>
          ...
          <div className="flex-1">
            ...
            <div className="flex gap-1 flex-wrap">
              {Array.from({ length: cols }, (_, i) => i + 1).map((v) => {
                ...
                return (
                  <button key={v} onClick={() => onChange({ colSpan: v === 1 ? undefined : v })}
                    className="flex-1 text-[10px] py-1 rounded"
          ...
          <div className="flex-1">
            ...
            <div className="flex gap-1 flex-wrap">
              {Array.from({ length: rows }, (_, i) => i + 1).map((v) => {
                ...
                return (
                  <button key={v} onClick={() => onChange({ rowSpan: v === 1 ? undefined : v })}
                    className="flex-1 text-[10px] py-1 rounded"
                    style={{ background: active ? 'var(--accent)' : 'var(--app-bg)', color: active ? '#fff' : 'var(--text-secondary)', border: `1px solid ${active ? 'var(--accent)' : 'var(--app-border)'}`, minWidth: 24 }}>
                    ×{v}
                  </button>
          ...
          <div>
            ...
            <div className="flex gap-1">
              {([false, true] as const).map((v) => {
                ...
                return (
                  <button key={String(v)} onClick={() => onChange({ allowOverflow: v || undefined })}
                    className="flex-1 text-[10px] py-1 rounded"
                    style={{ background: active ? 'var(--accent)' : 'var(--app-bg)', color: active ? '#fff' : 'var(--text-secondary)', border: `1px solid ${active ? 'var(--accent)' : 'var(--app-border)'}` }}>
