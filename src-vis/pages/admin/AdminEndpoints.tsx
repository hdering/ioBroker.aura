...
function DatapointPicker({ onAdd, onClose }: { onAdd: (dps: GroupDatapoint[]) => void; onClose: () => void }) {
        ...
        for (const device of devices) {
            for (const state of device.states) {
                ...
                result.push({
                    ...
                    type:
                        state.obj.common.type === 'boolean'
                            ? 'boolean'
                            : state.obj.common.type === 'number'
                              ? 'number'
                              : 'string',
                    unit: state.unit,
    ...
    return (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
            <div
                className="rounded-xl w-full max-w-2xl max-h-[80vh] flex flex-col shadow-2xl"
                        ...
                        {loading ? (
                            <>
                                ...
                                <p className="text-sm" style={{ color: 'var(--text-secondary)' }}>
                                    {t('endpoints.picker.loading')}
                                </p>
                            </>
                        ) : (
                            <button
                                onClick={load}
                                className="px-5 py-2 rounded-lg text-white text-sm hover:opacity-80"
                                style={{ background: 'var(--accent)' }}
                            >
                                {t('endpoints.picker.load')}
                            </button>
                        )}
                    </div>
                ) : (
                    <>
                        <div className="flex gap-2 px-5 py-3" style={{ borderBottom: '1px solid var(--app-border)' }}>
                            ...
                            >
                                ...
                                <input
                                    ...
                                    placeholder={t('endpoints.picker.search')}
                                    ...
                                    >
                                        ...
                                        >
                                            <input
                                                ...
                                                checked={allChecked}
                                                ref={(el) => {
                                                    if (el) el.indeterminate = deviceSelected > 0 && !allChecked;
                                                }}
                                            ...
                                            />
                                            <button
                                                className="flex-1 flex items-center gap-2 text-left hover:opacity-80 min-w-0"
                                            ...
                                            >
                                                ...
                                                >
                                                    {deviceSelected > 0 ? `${deviceSelected}/` : ''}
                                                    {device.states.length} DP
                                                </span>
                                            </button>
                                        </div>
                                        {expanded === device.id &&
                                            device.states.map((state) => (
                                                <label
                                                    ...
                                                    style={{
                                                        ...
                                                        background: selected.has(state.id)
                        ...
                        >
                            <span className="text-sm" style={{ color: 'var(--text-secondary)' }}>
                                {selected.size === 0
                                    ? t('endpoints.picker.none')
                                    : t('endpoints.picker.selected', { count: String(selected.size) })}
                            </span>
    ...
    return (
        <div>
            <div className="flex items-center justify-between mb-3">
                <span className="text-sm" style={{ color: 'var(--text-secondary)' }}>
                    {t('endpoints.dp.count', { count: String(group.datapoints.length) })}
                </span>
                <button
                        ...
                        >
                            ...
                            >
                                ...
                                >
                                    {dp.type === 'boolean'
                                        ? dp.writable
                                            ? t('endpoints.dp.typeSwitch')
                                            : t('endpoints.dp.typeBool')
                                        : dp.type === 'number'
                                          ? `${t('endpoints.dp.typeNum')}${dp.unit ? ` · ${dp.unit}` : ''}`
                                          : 'Text'}
                                </span>
                                <button
                                    onClick={() => setEditingId(editingId === dp.id ? null : dp.id)}
                                ...
                                <button
                                    onClick={() => removeDatapoint(group.id, dp.id)}
                                    className="hover:opacity-70 shrink-0"
                                ...
                                >
                                    ...
                                    <div className="flex items-center gap-2">
                                        ...
                                        <input
                                            ...
                                            onBlur={(e) =>
                                                updateDatapoint(group.id, dp.id, { label: e.target.value || dp.label })
                                            }
                                    ...
                                    <div className="flex items-center gap-2">
                                        ...
                                        <select
                                            value={dp.type}
                                            onChange={(e) =>
                                                updateDatapoint(group.id, dp.id, {
                                                    type: e.target.value as GroupDatapoint['type'],
                                                })
                                            }
                                            className="flex-1 text-xs rounded-lg px-2.5 py-1.5 focus:outline-none"
                                            style={inputStyle}
                                    ...
                                    {(dp.type === 'boolean' || dp.type === 'number') && (
                                        <div className="flex items-center gap-2">
                                            ...
                                            >
                                                {dp.type === 'boolean'
                                                    ? t('endpoints.dp.asSwitch')
                                                    : t('endpoints.dp.writable')}
                                            </label>
                                            <button
                                                onClick={() =>
                                                    updateDatapoint(group.id, dp.id, { writable: !dp.writable })
                                                }
                                                className="relative w-9 h-5 rounded-full transition-colors shrink-0"
                                                style={{
                                                    background: dp.writable ? 'var(--accent)' : 'var(--app-border)',
                                                }}
                                            >
                                                <span
                                            ...
                                            <span className="text-xs" style={{ color: 'var(--text-secondary)' }}>
                                                {dp.writable
                                                    ? dp.type === 'boolean'
                                                        ? t('endpoints.dp.toggle')
                                                        : t('endpoints.dp.settable')
            ...
            {groups.length === 0 ? (
                ...
                >
                    <p className="text-lg font-medium mb-1" style={{ color: 'var(--text-secondary)' }}>
                        {t('endpoints.noGroups')}
                    </p>
                    <p className="text-sm" style={{ color: 'var(--text-secondary)' }}>
                        {t('endpoints.noGroupsHint')}
                    </p>
                </div>
            ) : (
