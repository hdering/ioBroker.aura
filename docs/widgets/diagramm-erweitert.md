...
| `echartSeries[].color`         | Palette      | Linien-/Balkenfarbe als fester Wert (`#eab308`, `rgb(...)`, `red`) — **kein** `var(--token)`: das Diagramm zeichnet auf ein Canvas, dort löst der Browser keine CSS-Variable auf (auch nicht mit Fallback) und die Serie bleibt unsichtbar. Ohne Angabe die nächste Farbe der Palette. |
