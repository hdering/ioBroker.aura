...
Rechnet zwischen der Einheit des Datenpunkts und der Einheit um, in der bedient wird — in **beide** Richtungen: gelesen wird `Wert × Faktor + Offset`, geschrieben `(Eingabe − Offset) ÷ Faktor`. `minValue`, `maxValue`, `step` und `unit` gelten dann in der umgerechneten Einheit, der Datenpunkt behält seine eigene.
