...
export function renderOption(key, o) {
...
export function renderTypeIndex(schema) {
...
function renderNamedType(name, t) {
    if (t.enum) {
    ...
    if (t.fields) {
        const lines = Object.entries(t.fields).map(
            ([k, f]) =>
                `    ${k}${f.required ? '' : '?'}: ${typeName(f)}${f.description ? `  // ${f.description}` : ''}`,
        );
        return `${name} = {\n${lines.join('\n')}\n}`;
    }
    if (t.tuple) {
...
export function renderTypeDetail(types, schema) {
...
export function renderWidgetShape(schema) {
    return Object.entries(schema.widgetConfig)
        .map(([k, o]) => renderOption(k, o))
        .join('\n');
}
